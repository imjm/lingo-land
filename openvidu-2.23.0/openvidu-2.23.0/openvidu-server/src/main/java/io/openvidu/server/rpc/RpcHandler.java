...
public class RpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	private void sendMessage(RpcConnection rpcConnection, Request<JsonObject> request) {
		...
		log.info("{}", type);
		if(type != null && "signal:gameEnd".equals(type)) {
			String data = messageJson.get("data").getAsString();
			log.info("{}", data);
			sendGameEndInfoToServer(data);
		}

		sessionManager.sendMessage(participant, message, request.getId());
	}

	private void sendGameEndInfoToServer(String data) {
		String url = "http://localhost:8080/api/v1/game-end";
		try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
			...
			try (CloseableHttpResponse response = httpClient.execute(request)) {
				...
				System.out.println("ㅇㅇ" + statusCode);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
